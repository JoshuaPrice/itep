...
# print(dfConversions.index)
tfNcbiSeries = pd.Series(dfConversions.index)
...
print "Percent of TF from TcoF-DB in Ensembl database: ", float(numMatch)/totNum
...
print "Percent of TF from TcoF-DB+Ensembl in RNA-seq: ", float(numMatchRna)/totNumRna

sortedTf = dfTfList.sort_values(by=['RTCC'], ascending=False)

# Save to csv file
sortedTf.to_csv('data/sortedTF.csv',index=True,header=False)